...
function visualize(data) {
  selectedColumns = preAnalyse(data);
  let nested = nest(selectedColumns);
  let scales = make_scales(selectedColumns, margin);
...
function visualize1(data) {
  ...
  function mouseover(d) {
    ...
    updateLineChart(d.properties.county);
  }

  function updateLineChart(selectedCounty) {
    let filteredData = selectedColumns.filter(d => d.county === selectedCounty);

    d3.select("#lines")
      .selectAll("path")
      .remove();

    let scales = make_scales(filteredData, margin);
    draw_lines([filteredData], scales);
    draw_axes(scales, margin);
  }
}
